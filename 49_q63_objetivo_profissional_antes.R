...
# Script 49: 49_q63_objetivo_profissional_antes.R - Metas Pré-Ingresso
# ==============================================================================
...
df_q63 <- df |>
  mutate(
    ...
    objetivo_antes = factor(q63, levels = c(
      "Entrar no mercado de trabalho",
      "Melhorar de emprego",
      "Continuar estudando",
      "Empreender",
      "Não tinha objetivo definido"
    ))
  )

df_resumo_q63 <- df_q63 |>
  count(AnoCol, objetivo_antes) |>
...
tabela_q63_dados <- tibble(
  `Maior Objetivo Profissional Antes do PPE` = c(as.character(df_resumo_q63$objetivo_antes), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q63$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q63$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q63$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q63$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q63$dif_pp), "—")
)

t63_objetivo_antes <- tt(tabela_q63_dados) |>
  format_tt(escape = TRUE)

df_plot_q63 <- df_q63 |>
  count(AnoCol, objetivo_antes, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_objetivo_antes <- ggplot(df_plot_q63, aes(x = fct_rev(objetivo_antes), y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Aspiração de Origem: Maior Objetivo Profissional Antes de Entrar no PPE",
    subtitle = "Foco hegemônico na empregabilidade: entrar ou melhorar de emprego reuniu ~85-88%
(Cinza: 2025 | Azul: 2026)",
    x = "Objetivo Profissional Pré-Ingresso",
