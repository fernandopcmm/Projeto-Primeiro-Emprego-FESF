...
# Script: gest_12_q10_qualidade_comunicacao_parceira.R - Comunicação com Parceira
# ==============================================================================
...
df_q10 <- df |>
  mutate(
    ...
    escala = factor(q10, levels = 1:5, labels = c(
      "1 - Muito insatisfatório", "2 - Insatisfatório", "3 - Regular / Neutro",
      "4 - Satisfatório", "5 - Muito satisfatório"
    ))
  )

tabela_q10_dados <- tibble(
  `Qualidade da Comunicação com a Entidade Parceira` = c(
    "1 - Muito insatisfatório", "2 - Insatisfatório", "3 - Regular / Neutro",
    "4 - Satisfatório", "5 - Muito satisfatório", "Total"
  ),
  `2025 (N)` = c("6", "3", "12", "28", "18", "67"),
  `2025 (%)` = c("9,0%", "4,5%", "17,9%", "41,8%", "26,9%", "100,0%"),
  `2026 (N)` = c("2", "3", "18", "28", "26", "77"),
  `2026 (%)` = c("2,6%", "3,9%", "23,4%", "36,4%", "33,8%", "100,0%"),
  `Variação (p.p.)` = c("-6,4", "-0,6", "+5,5", "-5,4", "+6,9", "—")
)

t10_qualidade_comunicacao <- tt(tabela_q10_dados) |>
...
g10_qualidade_comunicacao <- ggplot(df_plot_q10, aes(x = escala, y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Articulação Interinstitucional: Qualidade da Comunicação com a Entidade Parceira",
    subtitle = "Recuo da insatisfação (-7,0 p.p.) e expansão das notas máximas (33,8% em 2026, +6,9 p.p.)
(Cinza: 2025 | Azul: 2026)",
    x = "Escala de Avaliação da Comunicação",
