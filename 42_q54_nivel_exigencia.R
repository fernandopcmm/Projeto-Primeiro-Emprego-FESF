...
# Script 42: 42_q54_nivel_exigencia.R - Nível de Exigência e Complexidade
# ==============================================================================
...
df_resumo_q54 <- df_q54 |>
  count(AnoCol, exigencia) |>
...
tabela_q54_dados <- tibble(
  `Nível de Exigência do Trabalho` = c(as.character(df_resumo_q54$exigencia), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q54$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q54$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q54$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q54$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q54$dif_pp), "—")
)

t54_exigencia <- tt(tabela_q54_dados) |>
  format_tt(escape = TRUE)

df_plot_q54 <- df_q54 |>
  count(AnoCol, exigencia, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_exigencia <- ggplot(df_plot_q54, aes(x = fct_rev(exigencia), y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Equilíbrio de Cobrança: Percepção do Nível de Exigência das Metas",
    subtitle = "Calibragem pedagógica exemplar: 82% a 88% consideram a exigência na medida exata
(Cinza: 2025 | Azul: 2026)",
    x = "Avaliação da Cobrança de Trabalho",
