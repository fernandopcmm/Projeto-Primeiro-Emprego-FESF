...
# Script 43: 43_q55_cultura_geral.R - Aquisição de Cultura Geral
# ==============================================================================
...
df_q55 <- df |>
  mutate(
    ...
    cultura = factor(q55, levels = c("Contribui amplamente", "Contribui parcialmente", "Contribui muito pouco", "Não contribui"))
  )

df_resumo_q55 <- df_q55 |>
  count(AnoCol, cultura) |>
...
tabela_q55_dados <- tibble(
  `Contribuição para Aquisição de Cultura Geral` = c(as.character(df_resumo_q55$cultura), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q55$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q55$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q55$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q55$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q55$dif_pp), "—")
)

t55_cultura <- tt(tabela_q55_dados) |>
  format_tt(escape = TRUE)

df_plot_q55 <- df_q55 |>
  count(AnoCol, cultura, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_cultura <- ggplot(df_plot_q55, aes(x = cultura, y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Repertório Cultural: Contribuição do Trabalho para Cultura Geral",
    subtitle = "Reconhecimento pedagógico expressivo: mais de 91% atestam ganhos culturais no PPE
(Cinza: 2025 | Azul: 2026)",
    x = "Grau de Contribuição Declarado",
