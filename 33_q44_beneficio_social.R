...
# Script 33: 33_q44_beneficio_social.R - Acesso a Benefícios Sociais
# ==============================================================================
...
df_q44 <- df |>
  mutate(
    ...
    ben_cat = case_when(
      grepl("bolsa f", q44, ignore.case = TRUE) ~ "Bolsa Família",
      grepl("bpc", q44, ignore.case = TRUE) ~ "BPC / LOAS",
      grepl("aposent", q44, ignore.case = TRUE) ~ "Previdência / Aposentadoria",
      grepl("nenhum|não|nao|^\\.$", q44, ignore.case = TRUE) ~ "Nenhum benefício social",
      TRUE ~ "Outros benefícios"
    ),
    ben_cat = factor(ben_cat, levels = c(
...
df_resumo_q44 <- df_q44 |>
  count(AnoCol, ben_cat) |>
...
tabela_q44_dados <- tibble(
  `Benefício Social Recebido pela Família` = c(as.character(df_resumo_q44$ben_cat), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q44$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q44$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q44$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q44$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q44$dif_pp), "—")
)

t44_beneficio <- tt(tabela_q44_dados) |>
  format_tt(escape = TRUE)

df_plot_q44 <- df_q44 |>
  count(AnoCol, ben_cat, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_beneficio <- ggplot(df_plot_q44, aes(x = fct_rev(ben_cat), y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Proteção Social: Acesso a Programas de Transferência de Renda",
    subtitle = "Cerca de 22% a 25% das famílias recebem Bolsa Família; ~3% acessam o BPC
