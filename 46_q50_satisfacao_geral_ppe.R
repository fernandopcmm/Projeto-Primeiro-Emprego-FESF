...
# Script 46: 46_q50_satisfacao_geral_ppe.R - Satisfação Geral com o PPE
# ==============================================================================
...
df_q50 <- df |>
  mutate(
    ...
    satisfacao = factor(q50, levels = c(
      "Muito satisfeito",
      "Satisfeito a maior parte do tempo",
      "Satisfeito somente em alguns momentos",
      "Insatisfeito"
    ))
  )

df_resumo_q50 <- df_q50 |>
  count(AnoCol, satisfacao) |>
...
tabela_q50_dados <- tibble(
  `Nível de Satisfação Geral com o Trabalho` = c(as.character(df_resumo_q50$satisfacao), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q50$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q50$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q50$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q50$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q50$dif_pp), "—")
)

t50_satisfacao <- tt(tabela_q50_dados) |>
  format_tt(escape = TRUE)

df_plot_q50 <- df_q50 |>
  count(AnoCol, satisfacao, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_satisfacao <- ggplot(df_plot_q50, aes(x = satisfacao, y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Avaliação Global: Nível de Satisfação Geral com o Trabalho no PPE",
    subtitle = "Consagração do programa: 94% a 95% avaliam a experiência como plenamente satisfatória
(Cinza: 2025 | Azul: 2026)",
    x = "Grau de Satisfação Declarado",
