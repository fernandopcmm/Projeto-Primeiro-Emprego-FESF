...
# Script 44: 44_q56_cidadao_exemplar.R - Formação Cívica e Cidadão Exemplar
# ==============================================================================
...
df_q56 <- df |>
  mutate(
    ...
    cidadania = factor(q56, levels = c("Contribui amplamente", "Contribui parcialmente", "Contribui muito pouco", "Não contribui"))
  )

df_resumo_q56 <- df_q56 |>
  count(AnoCol, cidadania) |>
...
tabela_q56_dados <- tibble(
  `Contribuição para Ser um Cidadão Exemplar` = c(as.character(df_resumo_q56$cidadania), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q56$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q56$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q56$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q56$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q56$dif_pp), "—")
)

t56_cidadao <- tt(tabela_q56_dados) |>
  format_tt(escape = TRUE)

df_plot_q56 <- df_q56 |>
  count(AnoCol, cidadania, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_cidadao <- ggplot(df_plot_q56, aes(x = cidadania, y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Formação Cívica: Contribuição do Programa para a Cidadania Ativa",
    subtitle = "Consagração formativa: ~79-82% afirmam que o PPE contribui amplamente para a cidadania
(Cinza: 2025 | Azul: 2026)",
    x = "Grau de Contribuição Cidadã Declarado",
