...
# Script 39: 39_q51_auxilio_carreira.R - Auxílio da Instituição na Progressão
# ==============================================================================
...
df_q51 <- df |>
  mutate(
    ...
    auxilio = factor(q51, levels = c("Plenamente", "Parcialmente", "Pouco", "Não sinto que estou progredindo"))
  )

df_resumo_q51 <- df_q51 |>
  count(AnoCol, auxilio) |>
...
tabela_q51_dados <- tibble(
  `Instituição Auxilia na Progressão da Carreira` = c(as.character(df_resumo_q51$auxilio), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q51$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q51$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q51$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q51$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q51$dif_pp), "—")
)

t51_carreira <- tt(tabela_q51_dados) |>
  format_tt(escape = TRUE)

df_plot_q51 <- df_q51 |>
  count(AnoCol, auxilio, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_carreira <- ggplot(df_plot_q51, aes(x = auxilio, y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Desenvolvimento Profissional: Suporte Institucional à Progressão na Carreira",
    subtitle = "Expressiva maioria percebe suporte efetivo (64-69% plenamente e 26-29% parcialmente)
(Cinza: 2025 | Azul: 2026)",
    x = "Percepção de Auxílio no Desenvolvimento",
