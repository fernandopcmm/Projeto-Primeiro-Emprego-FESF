...
# Script 40: 40_q52_recursos_tecnologicos.R - Uso de Tecnologias no Posto
# ==============================================================================
...
df_q52 <- df |>
  mutate(
    ...
    tecnologia = case_when(
      grepl("Amplo e adequado", q52) ~ "Amplo e adequado",
      grepl("Restrito, mas adequado", q52) ~ "Restrito, mas adequado",
      grepl("Amplo, mas inadequado", q52) ~ "Amplo, mas inadequado",
      grepl("não dispões", q52) ~ "Organização não dispõe de recursos",
      grepl("Restrito e inadequado", q52) ~ "Restrito e inadequado",
      TRUE ~ "Outra avaliação"
    ),
    tecnologia = factor(tecnologia, levels = c(
      "Amplo e adequado",
      "Restrito, mas adequado",
      "Amplo, mas inadequado",
      "Organização não dispõe de recursos",
      "Restrito e inadequado"
    ))
  )

df_resumo_q52 <- df_q52 |>
  count(AnoCol, tecnologia) |>
...
tabela_q52_dados <- tibble(
  `Acesso e Domínio de Tecnologias no Trabalho` = c(as.character(df_resumo_q52$tecnologia), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q52$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q52$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q52$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q52$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q52$dif_pp), "—")
)

t52_tecnologia <- tt(tabela_q52_dados) |>
  format_tt(escape = TRUE)

df_plot_q52 <- df_q52 |>
  count(AnoCol, tecnologia, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_tecnologia <- ggplot(df_plot_q52, aes(x = fct_rev(tecnologia), y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Inclusão Digital no Trabalho: Uso de Recursos Audiovisuais e Tecnologias",
    subtitle = "Cerca de 78% a 80% desfrutam de ambiente tecnológico amplo e adequado nas repartições
(Cinza: 2025 | Azul: 2026)",
    x = "Avaliação da Infraestrutura Tecnológica",
