...
# Script 48: 48_q62_crenca_emprego_formal_previo.R - Crença Prévia de Inserção Formal
# ==============================================================================
...
tabela_q62_dados <- tibble(
  `Acreditava que Conseguiria Emprego Formal Antes do PPE?` = c(as.character(df_resumo_q62$crenca), "Total"),
  `2025 (N)` = c(as.character(df_resumo_q62$`2025`), "711"),
  `2025 (%)` = c(percent(df_resumo_q62$pct_2025, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `2026 (N)` = c(as.character(df_resumo_q62$`2026`), "879"),
  `2026 (%)` = c(percent(df_resumo_q62$pct_2026, accuracy = 0.1, decimal.mark = ","), "100,0%"),
  `Variação (p.p.)` = c(sprintf("%+.1f", df_resumo_q62$dif_pp), "—")
)

t62_expectativa <- tt(tabela_q62_dados) |>
  format_tt(escape = TRUE)

df_plot_q62 <- df_q62 |>
  count(AnoCol, crenca, name = "Respondentes") |>
  group_by(AnoCol) |>
  mutate(
...
g_expectativa <- ggplot(df_plot_q62, aes(x = crenca, y = Percentual, fill = AnoCol)) +
  ...
  labs(
    title = "Expectativa Prévia: Crença de Inserção no Mercado Formal Antes do PPE",
    subtitle = "Quase 80% viam barreiras severas (50-55% viam muita dificuldade e 23-25% achavam improvável)
(Cinza: 2025 | Azul: 2026)",
    x = "Crença Prévia Declarada",
